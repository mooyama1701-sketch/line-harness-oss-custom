# create-line-harness-custom Sandbox Runner

`@airestart/create-line-harness` writes state into `~/.line-harness-custom`, reads wrangler auth from your home/XDG config, and clones the custom OSS repo into a canonical location. When you want to reproduce installer bugs on the same MacBook without contaminating your normal setup, run the CLI inside a relocated `HOME`.
...
- `~/.line-harness-custom` clone and `.line-harness-custom-setup.json`
- wrangler config/cache under `HOME` and `XDG_*`
- npm user config, npm cache, pnpm home, corepack cache
- the working directory from which `npx @airestart/create-line-harness` is launched
...
Local isolation prevents the installer from touching your real `~/.line-harness-custom` and wrangler session files. It does **not** make Cloudflare deploys harmless by itself. For a true repro lane, combine the sandbox with:
...
```bash
cd /Users/axpr/claudecode/tools/line-harness
./scripts/run-create-line-harness-sandbox.sh --name repro-20260521 --reset -- npx @airestart/create-line-harness
```
...
4. After the run, inspect only the sandbox copy:
   - `$LINE_HARNESS_SANDBOX_ROOT/home/.line-harness-custom`
   - `$LINE_HARNESS_SANDBOX_ROOT/home/.line-harness-custom/.line-harness-custom-setup.json`
   - `$LINE_HARNESS_SANDBOX_ROOT/home/.line-harness-custom/apps/worker/wrangler.toml`
